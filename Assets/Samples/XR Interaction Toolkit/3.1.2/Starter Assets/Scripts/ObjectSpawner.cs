...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets
{
    ...
    public class ObjectSpawner : MonoBehaviour
    {
        ...
        // Keep only the most recently spawned object
        [SerializeField, Tooltip("If true, destroy the previously spawned object before placing a new one.")]
        bool m_KeepOnlyLatest = true;

        // Reference to the current (most recent) spawned object
        GameObject m_LastSpawned;
        ...
        public bool TrySpawnObject(Vector3 spawnPoint, Vector3 spawnNormal)
        {
            ...
            // If we only want the latest, delete whatever we spawned last time
            if (m_KeepOnlyLatest)
            {
                if (m_LastSpawned != null)
                {
                    Destroy(m_LastSpawned);
                    m_LastSpawned = null;
                }

                // Extra safety when "Spawn As Children" is used — remove any old children this spawner created.
            ...
            // Remember the newest object so we can destroy it next time
            m_LastSpawned = newObject;
